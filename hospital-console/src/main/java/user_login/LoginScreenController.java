...
public class LoginScreenController {
  ...
  public void login(ActionEvent event) throws Exception
	{
		boolean loginStatus = LoginScreenController.validateUserAndPassword(userName.getText(), mobile.getText());
		if(loginStatus)
		{
	...
	public static boolean validateUserAndPassword(String userName, String mobile)
	{
		...
		try {
			response = RestUtilGenerics.sendPostRequest("http://localhost:8081/api/v1/directory/validate", LoginResponse.class, loginRequest);
